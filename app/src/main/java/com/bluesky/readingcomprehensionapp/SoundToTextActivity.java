...
public class SoundToTextActivity extends Activity {
    ...
    private void ball() {
        if (loaded && !plays) {
            soundPool.play(ball, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void bear() {
        if (loaded && !plays) {
            soundPool.play(bear, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void bee() {
        if (loaded && !plays) {
            soundPool.play(bee, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void book() {
        if (loaded && !plays) {
            soundPool.play(book, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void bus() {
        if (loaded && !plays) {
            soundPool.play(bus, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void car() {
        if (loaded && !plays) {
            soundPool.play(car, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void cat() {
        if (loaded && !plays) {
            soundPool.play(cat, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void circle() {
        if (loaded && !plays) {
            soundPool.play(circle, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void cow() {
        if (loaded && !plays) {
            soundPool.play(cow, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void dog() {
        if (loaded && !plays) {
            soundPool.play(dog, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void door() {
        if (loaded && !plays) {
            soundPool.play(door, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void duck() {
        if (loaded && !plays) {
            soundPool.play(duck, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void egg() {
        if (loaded && !plays) {
            soundPool.play(egg, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void fish() {
        if (loaded && !plays) {
            soundPool.play(fish, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void flower() {
        if (loaded && !plays) {
            soundPool.play(flower, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void frog() {
        if (loaded && !plays) {
            soundPool.play(frog, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void goat() {
        if (loaded && !plays) {
            soundPool.play(goat, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void hat() {
        if (loaded && !plays) {
            soundPool.play(hat, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void house() {
        if (loaded && !plays) {
            soundPool.play(house, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void leaf() {
        if (loaded && !plays) {
            soundPool.play(leaf, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void lion() {
        if (loaded && !plays) {
            soundPool.play(lion, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void moon() {
        if (loaded && !plays) {
            soundPool.play(moon, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void pie() {
        if (loaded && !plays) {
            soundPool.play(pie, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void ship() {
        if (loaded && !plays) {
            soundPool.play(ship, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void sky() {
        if (loaded && !plays) {
            soundPool.play(sky, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void star() {
        if (loaded && !plays) {
            soundPool.play(star, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void sun() {
        if (loaded && !plays) {
            soundPool.play(sun, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void square() {
        if (loaded && !plays) {
            soundPool.play(square, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void tree() {
        if (loaded && !plays) {
            soundPool.play(tree, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
    ...
    private void triangle() {
        if (loaded && !plays) {
            soundPool.play(triangle, volume, volume, 1, 0, 1f);
            plays = false;
        }
    }
